...
modified: 2026-07-20T15:16:20.868-03:00
tags:
  - unfinished
---
...
Random number generating is a practice that creates random data. The most common way to generate random-sh numbers is the _PRNG_ (_Pseudo-Random Number Generator_) and the _CSPRNG_ (_Cryptographic-Secure Pseudo-Random Number Generator_).
...
For generating PRNGs, the algorithm is mostly simple, at least in common PRNG algorithms, as CSPRNGs're much more complex. In this note we'll study the `LCG `, the `xorshift` (including the `+`/`*` versions and the non-linear `xorwow`), the `SplitMix64` and the `xoshiro`/`xoroshiro`. After a brief overview, we'll develop our own algorithm (I'll name it as `uwu64`) and optimize it to pass `64GB PractRand` test with up to `2GB/s throughput` and a period equal or bigger than $2^{256}$.
...
![[Programming/Algorithms/Assets/Marsaglia_plot_lcg_unmixed.png]]
...
You might be asking "Why these specific values for the bit-wise right shifts and multiplication?", and you're not wrong for asking an answer to this question. First of all we should know: what do we want by using this mixer? We want to fix the mainly tests errors: `[LowK/64]`, that is a generic version of any of these errors: `[Low1/64]`, `[Low2/64]`, `[Low4/64]`, `[Low8/64]`, and so many more until the `[Low32/64]` (why not 64 you may ask, it's because only the first 32 bits, of the 64 bits word, is considered low bits, you can read more about it in [[Byte Endianness]]).

To fix they, we need to fix the bad distribution around the word, for this we firstly shift and xor the word:
...
The xor operation changes the LSBytes (as zeroes don't matter much in xor operations), and this is our $z$ value. After this, we start mixing over the new base value, the first operation is another shift + xor:
...
And then we use an uneven constant for multiplicative mixing:
...
And then a last shift + xor to get the mixed output
...
#### Why Uneven Constants?

The use of even constants for mixing isn't recommended, and here are some reasons:

1. **Information Loss when multiplying**: If you multiply by an even number in binary, you're effectively shifting left at least one bit

For example:

$$
x \cdot 6 = x \cdot \texttt{110} \text{ (in binary)}
$$

Since 6 is divisible by 2 (even)

$$
x \cdot 6 = (x \cdot 3) \ll 1
$$

That means the least significant bit (LSB) is always 0

$$
\texttt{10101101} \times \texttt{110} = \texttt{1111110110}
$$

2. **Even Numbers isn't invertible modulo $2^n$**: Most fast hashes and PRNGs Algorithms operate modulo $2^{64}$ or $2^{32}$. A multiplication is reversible only if the multiplier is odd, this comes from number theory:

$$
a \text{ has a inverse mod } 2^n
$$
...
Since every even number shares a factor of $2$ with $2^n$:
...
Different inputs become identical after multiplication. Collisions become unavoidable.
...
This table shows just the first 8 GB, but the test ran until the 128 GB, before I stopped the test.

To follow the tradition, here is a Marsaglia representation of the mixed LCG:

![[Programming/Algorithms/Assets/marsaglia_plot_mixed_lcg.png|594]]
